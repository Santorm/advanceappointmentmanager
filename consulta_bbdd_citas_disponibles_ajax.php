...
$sql="SELECT citas_disponibles.id_cita, responsable.nombre_responsable, centro.ciudad, servicios.nombre_servicio, horario.hora_inicio, citas_disponibles.fecha, citas_disponibles.estado, pacientes.nombre, pacientes.apellidos
FROM citas_disponibles
JOIN responsable
ON responsable.id_responsable = citas_disponibles.id_responsable
JOIN centro
...
LEFT JOIN pacientes
ON pacientes.id_persona = citas_disponibles.id_persona
WHERE $filtro_responsable AND $filtro_centros AND $filtro_tratamiento AND $filtro_fecha AND $filtro_hora
...
array_push($arrayCitas, array('id_cita'=>$fila['id_cita'],'nombre_responsable'=>$fila['nombre_responsable'], 'ciudad'=>$fila['ciudad'], 'nombre_servicio'=>$fila['nombre_servicio'], 'hora_inicio'=>$fila['hora_inicio'], 'fecha'=>$fila['fecha'], 'estado'=>$fila['estado'], 'nombre'=>$fila['nombre'], 'apellidos'=>$fila['apellidos']));
